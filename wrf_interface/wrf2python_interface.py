...
header = """
extern void emulator_interface_(float *th,float *qv_curr,float *qc_curr,float *qr_curr,float *qi_curr,float *qs_curr,float *qg_curr,float *qni_curr,float *qns_curr,float *qnr_curr,float *qng_curr,float *rho,float *pi_phy,float *p,float *dt,float *dz8w,int *nx,int *ny,int *nz, int *ireturn);
...
@ffi.def_extern()
def emulator_interface_(cp_th,cp_qv_curr,cp_qc_curr,cp_qr_curr,cp_qi_curr,cp_qs_curr,cp_qg_curr,cp_qni_curr,cp_qns_curr,cp_qnr_curr,cp_qng_curr,cp_rho,cp_pi_phy,cp_p,cp_dt,cp_dz8w,nx,ny,nz,ireturn):
    nz_py=nz[0]
    ny_py=ny[0]
    nx_py=nx[0]
    dt=cp_dt[0]
    ...
    ireturn[0]=0
    return
    ireturn[0]=call_emulator(th,qv_curr,qc_curr,qr_curr,qi_curr,qs_curr,qg_curr,qni_curr,qns_curr,qnr_curr,qng_curr,rho,pi_phy,p,dt,dz8w,model)
